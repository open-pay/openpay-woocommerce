...
class Openpay_Spei extends WC_Payment_Gateway
{
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            'enabled' => array(
                ...
                'label' => __('Enable Openpay SPEI', 'woothemes'),
                'default' => 'yes'
            ),
            'sandbox' => array(
                'type' => 'checkbox',
    ...
    protected function processOpenpayCharge() {
        ...
        if ($result_json != false) {
            ...
            WC()->session->set('pdf_url', $this->pdf_url_base.'/'.$this->merchant_id.'/'.$result_json->id);
    ...
    protected function markAsFailedPayment() {
        $this->order->add_order_note(
                sprintf(
                        "%s SPEI Payment Failed with message: '%s'", $this->GATEWAY_NAME, $this->transactionErrorMessage
                )
        );
    }
...
//add_action('woocommerce_order_status_processing_to_completed', 'openpay_spei_order_status_completed');